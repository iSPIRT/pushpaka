...
#   end
  ...
  config.vm.define "jammy", autostart: false do |jammy|
    ...
    jammy.vm.provision :shell, path: "ardupilot/Tools/vagrant/initvagrant.sh"
    jammy.vm.provider "virtualbox" do |vb|
      vb.name = "ArduPilot (jammy)"
    end
    jammy.vm.boot_timeout = 1200
  end
  config.vm.define "jammy-desktop", primary: true do |jammy|
    jammy.vm.box = "ubuntu/jammy64"
    # jammy.vm.provision :shell, path: "ardupilot/Tools/vagrant/initvagrant-desktop.sh"
    jammy.vm.provider "virtualbox" do |vb|
